...
function draw() {
	...
	// Uncomment below to save 120 images of sketch animation.
	// !!! If ran with preset 4 (biggest gridSize) it might take up to 10 minutes !!!
	// Then one can upload images to gifmaker.me and see fluid animation
	...
	for (let x = 0; x < settings.gridSize; x++) {
		for (let y = 0; y < settings.gridSize; y++) {
			...
			if (x === 0 ||
				...
				let perlinNoise = noise(x * settings.detail, y * settings.detail, (frameCount * settings.zChangePerFrame) + zOffset);
				...
				if (between(perlinNoise, 0.4, 0.5)) {
					...
					point(pixelX, pixelY, sF);
				} else if (between(perlinNoise, 0.398, 0.4) ||
